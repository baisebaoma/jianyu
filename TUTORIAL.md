...
本人有编程基础。在 GitHub 上找到了新月\*项目，认为代码非常清晰并且易于扩展，从 2023 年 11 月 31 日开始接触扩展包开发，实现了武将若干，已经有一些经验了。这个文档是为了方便您想参考本扩展包来制作自己的扩展包的。我会尽可能使用通俗易懂的话，每个武将会讲一些内容。你可以先去扫一眼 `README.md` 知道我这个包里有什么武将和技能。
...
如果您有想制作的技能，新月\*中几乎一定有一个类似的技能（或一部分）已经被别人写过了，但是直接去阅读那个技能的代码并尝试抄过来的话会非常困难！因为您可能不知道很多部分是什么意思，如：`on_trigger` 代表什么含义？`Room:sendLog` 函数里的 `from` 参数到底是传 `number` 类型还是 `ServerPlayer` 类型？为什么一个技能又有 `can_trigger` 又有 `on_refresh`？
...
2. 完整阅读完新月\*的 Docs 文件夹，那里有不错的介绍。接手任何一个项目前我都建议您大致读一下它的文档，然后再慢慢开发，不懂的地方再看文档。不过新月\*的文档短小精炼，我认为完整阅读不花时间，并且收获颇丰，所以建议先完整阅读之；
...
我目前知道的最好的方法是：使用 VS Code 直接打开 `/FreeK\*\*\*/` 文件夹，然后想搜什么都能搜到。

## \*名规范

这是我的个人习惯。将你自己的技能都加一个前缀（如简浴包所有的技能名都有`jy_`前缀）。这是为了防止重名。在我制作武将李元浩的技能【虎啸】时，其他扩展包已经有一个叫作`huxiao`的技能了，所以报错。我决定在我自己的扩展包中全部加前缀，尽量避免和其他扩展包重名。
...
Fk:loadTranslationTable{
  ...
  ["jy_kaiju_2"] = "开局",
  [":jy_kaiju_2"] = "出牌阶段限一次，你选择若干名角色，依次视为你对其使用一张【顺手牵羊】，然后被其使用一张【\*】。",
  ...
  ["jy_xizao_2"] = "洗澡",
  [":jy_xizao_2"] = "限定技，处于濒\*状态且装备区有牌时，你可以弃置所有装备区的牌、将体力恢复至1，然后每以此法弃置一张牌，你摸一张牌。",
...
定义武将类，就是告诉新月\*：我这个包里有一个武将，名字叫啥，体力是多少，体力上限是多少，性别是什么，势力是什么。新月\*知道了，就会给你的包加一个这个武将。不过现在还没技能。
...
翻译表是把一些用于代码里面简洁的短字符串翻译成中文的东西。其实在任何一个项目里，我们都不建议直接把字符串写似在代码里，而是像新月\*一样做翻译表将它们独立出来会是更好的选择。既降低耦合度，又提升代码易读性，还方便以后修改和添加多语言之类的。你应该看上面的例子就能看得懂这个翻译表里哪些对应什么内容，在这里就不赘述了。

### 编写技能

这部分挺复杂的，在 FreeK\*\*\* 官方文档里有说明，请你先去阅读那里。

我需要强调的一点是：所有部分都是可以省略的。在官方文档以及代码中，新手可能会看不懂，【\*计】监听 `fk.Damaged` （伤\*造成后）事件，但为什么代码看起来这个事件就可以直接触发摸牌？难道所有人受到伤\*都可以触发【\*计】吗？这是因为郭嘉的【\*计】在官方代码中省略了 `can_trigger`、`on_trigger` 两个参数。省略的意思是默认的参数就已经能满足要求了。默认参数就已经包含了判断下列条件：确实是我受到的伤\*、我拥有这个技能。`on_cost` 中写了每受到一点伤\*就要发动一次，而 `on_use` 中写了他需要摸两张牌并且分配之。
...
举例：我想设计一个“\*\*技”，名字叫 `jy_yuanshen`，也就是说所有基于\*\*角色制作的武将都会有完全相同的这个技能。这种情况下，我需要先让我包里的第一个武将使用变量名来获得这个技能（如：`jy_raiden:addSkill(jy_yuanshen)`），然后其他武将使用它的名字来获得这个技能（如：`jy_ayato:addSkill("yuanshen")`）。

这两种添加技能的多态方法混用时，似乎不能指定技能的显示顺序。不过我觉得没什么所谓。如果你实在介意这个，你可以复制粘贴一遍那个技能，改个名字就行了（如果你要改配音的话，也可以一并修改之）。

#### 相关技能

有的时候也会使用 `General:addRelatedSkill()`，这个方法的目的是在武将一览页面中在末尾以紫色提示这个武将可能会拥有这个技能，而且这个技能在游戏开始时不属于自己（如邓艾【急\*】），不会对武将强度和技能产生什么效果。
...
以这段考\*大学生的代码为例，考\*大学生自己开局就有的技能是【做题】（`jy_zuoti`）、【熬夜】（`jy_jieju`），他的使\*技有可能会使他获得【集智】、【看破】、【享乐】、【玉玉】、【红温】，那么建议你把这些技能使用 `General:addRelatedSkill()` 添加给这个武将，来方便玩家如果不知道这些技能的效果，那么可以在这个武将的说明里知道这个技能的效果是什么样的。
...
>
> \*\*：回合开始阶段，你可以将一张手牌扣于你的武将牌上，视为“点”，当“点”花色不同时，你视为使用一张\*\*入侵，回合内仅限一次
>
...
2. 然后，‘当你失去体力时’，“失去体力”和“受到伤\*”是两个完全不同的事件，他自己又没有主动失去体力的技能，那就基本触发不了了。这个肯定不对。我的建议是改成受到伤\*，这样触发频率高得多，对标强度也说得过去。
...
我们翻译完，其实发现他想做的事情就是让“点”的数量变动时，检测花色和数量，如果花色不同就放**入侵（每回合限一次，他自己说的），如果数量为 4 就放万箭齐发。然后你受到伤\*时可以弃一张“点”，你准备阶段可以用一张手牌当“点”，你使用的第二张基本牌可以当“点”。这就是需求了。

把它翻译成三国\*的中文吧！有两个相似的需求，那就是检测花色和数量，并执行不同的效果，我建议把这两个需求合成一个技能。其他的变成规范的技能描述即可。
...
类似的还有高天亮。他之前说的是：受到来自队友的伤\*时，摸三张牌然后翻面。那如何定义队友？他跟我说，所有人都是你的队友，对你使用了\*造成了伤\*的就不是你的队友。所以我最后写成了下面这个版本，效果是一样的。

> 玉玉：
>
> 1. 锁定技，当有角色对你使用【\*】造成了伤\*时，其获得【致郁】标记；<br>
> 2. 受到没有【致郁】标记的角色或因本次伤\*而获得【致郁】标记的角色造成的伤\*时，你可以选择一项：摸三张牌；摸四张牌并翻面。

这样写的好处是不用给全场打太多标记。你这个技能的重要性也没有那么强，也就摸个三张，为啥开局给所有人打个标记呢？还不如反过来，谁不是你的队友就给谁上标记，这样不是合理多了。

这个工作看起来也太像阅读理解了吧。我觉得这种工作需要又精通三国\*，又精通代码，知道哪些需求能写出来、哪些需求写不出来。但总之，我们已经顺利把它转换成程序员喜欢的文字了！

## 考\*大学生：参考别人的代码与爬\*

这是我认为简浴包最天才的武将之一。这个武将很好地使用测试包的谋徐盛（有很多用于测试的技能）中的技能，结合做选择题（并且也实现了让全场玩家都可以看到正在做的题的题干和答案），使用爬\*技术结合人工处理数据，设计出了一个有强度、平衡、有实用价值（应该说最有实用价值的吧，毕竟哪个其他武将能帮你考\*？）、娱乐（随机性大但题目难度稳定、题库大）、玩家反馈合理（正反馈强、答错无惩罚）的武将。经过群友的强度测试，总体感觉是符合简浴包合理强度的，不至于让别的武将没法玩，使\*成功（但是很难）之后也能获得极强的正反馈。
...
>
> 熬夜：使\*技，出牌阶段限一次，你可以失去一点体力使【做题】可以再使用一次。<br>
>
...
我刚实现这个武将的时候，我是手动复制粘贴的题的，太慢了，花了好久才 100 多道题，很快就被群友把题库玩干净了。遂想到使用爬\*。

说实话我对爬\*一窍不通，上次爬东西可能是三四年前了（爬 `op.gg` 的英雄数据并且获得合适的出装，当时还天天沉迷英雄联盟，服了）。我直接让 C\*\*\*\*PT 帮我搞的。给它看一部分你的 html 文件，告诉它你要什么样的输出（在我这里，我需要 Lua 的 table 形式）让它生成代码来帮你爬，哪里不对告诉它，它会改。最终跟它一起琢磨了两个多小时，就搞好了，我基本上自己没怎么写代码。你可以去 `packages/jianyu/questions/pachong.py` 看到这段代码，并在相同文件夹下看到所有爬到的试卷。我已经抽样调查过了，就算题目可能有一点格式问题，但答案确实是正确的。

当然还是有些问题的，比如题目里面的下划线用的是\<u\>标签，我爬不到；百分号和英文冒号会被认成通配符导致不显示后面的字了；图片题做不了，需要删除；过时题和材料题建议删掉。这些手动解决一下吧，反正一张卷子也就几分钟就搞好了，不碍事。让 C\*\*\*\*PT 搞半天他也搞不好。

我觉得都 2023 年了，是时候学会让 AI 来帮助你完成任务了。一些小技巧就是你得描述得足够仔细，它才能输出你想要的东西，不然它可能理解不到。

## 雷电\*\*：平衡武将

这个\*\*角色改编成三国\*武将看起来没什么难度，核心机制就是给队友挂雷眼，队友造成伤\*的时候我也造成一次伤\*，然后可以在合适的时候开大打伤\*。

一开始我的设计是每回合给一名角色挂雷眼，有雷眼的角色造成伤\*时判定，若为♣就造成1点雷伤并获得一个愿力。然后每回合可以消耗所有愿力打伤\*，有多少愿力就造成多少伤\*+1。后来经过PVE测试发现节奏太慢了，看起来很厉害，但实际上没什么用（尤其是在活动服环境里）。调整了很多天，中间也研究过弃牌开大什么的，但感觉对一个无法额外摸牌的武将来说有点垃\*。最后改成了红色给愿力黑色1点雷伤，开大只造成1点伤\*，但给所有有雷眼的人有多少愿力发多少张牌。

这样就几乎成了一个纯粹的辅助，给队友增伤发牌，无条件制伤，牌好的情况下自己也有一定输出能力（指一回合打三四点）。我承认确实可能有点强了，但强度这个东西，不得已而为之。大家都强，作为简浴包的第一个神将，当然强度要给高一点。总的来说我觉得设计还是比较成功的，但我觉得可能要把四体力改成三体力，不然不开其他DIY将的场上出现一个这个东西太超模了，伤\*过高。之后再观察一段时间吧。

## 神里绫人：改编武将

您可以想一下如果您来改编这个角色到《三国\*》里，您会如何设计他的技能：普攻没什么用，小技能是位移一小段，然后加强普通攻击变成大范围水伤，大招是让自己周围区域随机落水花造成伤\*，被动一是给小技能按生\*值增伤，被动二是自己处于后台且蓝低于一半时回大招的蓝。\*座一是对生\*值低于一半的目标伤\*提升，\*座二是增加小技能生\*值倍率、开小技能获得生\*值，后面的就不用介绍了。

我觉得不太好设计。毕竟《三国\*》武将绝大多数只有个位数的体力，每次增伤都很致\*；大招如果做成随机伤\*，又不可控，打谁？怎么选目标？怎么造成伤\*？持续多久？我想了好久也没想明白（后面我发现雷刹张角这个武将对这个随机伤\*实现得挺好的），也许可以设计成【定时炸弹】那样，但我觉得随机打击角色的设计感一般，给玩家的正反馈也不强。

我的设计一般都是技能描述越简洁越好，能让玩家一下就看得懂，不然阅读个几百字挺痛苦的（我很不喜欢！）。最后我决定放弃大招，从小技能、被动二和\*座二着手，设计了现在这个版本。基本回合外使用基本牌就能摸，然后如果用了\*还能额外摸一张。如果能一直保持闪\*的节奏，能多摸很多牌，但是要是残体力了就寄。回合内也能额外出\*、加手牌上限，回合外能多造成伤\*，要是快似了还能一定程度上保\*。

强度是故意设计的不高的，因为我和朋友玩的时候基本上都没开强度特别\*\*的武将，我也不希望简浴包的武将强度膨胀得太快。我觉得杨藩（杨藩没什么好说的，除了难写没有别的特点了，不单开一节了）这个武将的强度就已经有点高了，4体力，掉体力大概率摸2张，觉醒之后加体力上限还巨强，这让本包第一个武将简自豪有点难绷，感觉就算是界简自豪也没杨藩强。不过这些都是群友设计的，强度不归我管，我只负责做。他要是设计个被闪电劈中直接胜利的武将，那我也得给他做。

对了，如果你需要提取官方的\*\*角色配音，我在代码的这部分里写了一段注释，你可以去看看。

## 高天亮：平衡武将

早期设计我已经在上面提过了，就不说了。没什么用，纯摸牌白板，后期单挑也是白板，所以决定加强技能。既然技能特色是摸牌，那就贯彻到底咯？狠狠地摸！

其实我觉得摸牌很多不是很厉害，反正摸牌白板也用不出去，而且高天亮的设计是被【\*】了就不能继续从这个人的伤\*里摸了。
