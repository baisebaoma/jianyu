...
local muhuo = fk.CreateTriggerSkill {
  ...
  can_trigger = function(self, event, target, player, data)
    return player:hasSkill(self) and not target.dead and player:distanceTo(target) <= 1 and
        player:distanceTo(target) >= 0 and player:usedSkillTimes(self.name) == 0 -- 需要判断 >= 0，因为如果目标因该伤害而死就是-1了
  end,
  on_use = function(self, event, target, player, data)
...
Fk:loadTranslationTable {
  ...
  ["jy_muhuo"] = [[目祸]],
  [":jy_muhuo"] = [[每回合限一次，当一名角色受到伤害后，若你与其距离1以内且其未死亡，你可以摸X张牌（X为你已损失的体力值且至多为2）并将〖冒充〗改为阴。若其不为你，本回合其不是你使用牌的合法目标。]],
}
